...
tf.app.flags.DEFINE_string("data_dir",   "data/ITOP/", "Data directory")
...
def train():
  ...
  actions = data_utils_1.define_actions( FLAGS.action )
  ...
  train_set_3d, test_set_3d, data_mean_3d, data_std_3d, dim_to_ignore_3d, dim_to_use_3d = data_utils_1.read_3d_data(
    actions, FLAGS.data_dir, FLAGS.camera_frame,  FLAGS.predict_14 )

  # Read stacked hourglass 2D predictions if use_sh, otherwise use groundtruth 2D projections
  if FLAGS.use_sh:
    train_set_2d, test_set_2d, data_mean_2d, data_std_2d, dim_to_ignore_2d, dim_to_use_2d = data_utils_1.read_2d_predictions(actions, FLAGS.data_dir)
 # else:
#    train_set_2d, test_set_2d, data_mean_2d, data_std_2d, dim_to_ignore_2d, dim_to_use_2d = data_utils_1.create_2d_data( actions, FLAGS.data_dir, rcams )
  ...
  with tf.Session(config=tf.ConfigProto(
    ...
    for _ in xrange( FLAGS.epochs ):
      ...
      if FLAGS.evaluateActionWise:
        ...
        print("{0:=^12} {1:=^6}".format("Action", "m")) # line of 30 equal signs
        ...
        for i in range(n_joints):
          ...
          print("Error in joint {0:02d} (m): {1:>5.2f}".format(i+1, joint_err[i]))
        print("=============================")
...
def evaluate_batches( sess, model,
  ...
  for i in range(nbatches):
    ...
    # denormalize
    enc_in  = data_utils_1.unNormalizeData( enc_in,  data_mean_2d, data_std_2d, dim_to_ignore_2d )
    dec_out = data_utils_1.unNormalizeData( dec_out, data_mean_3d, data_std_3d, dim_to_ignore_3d )
    poses3d = data_utils_1.unNormalizeData( poses3d, data_mean_3d, data_std_3d, dim_to_ignore_3d )
...
#    train_set_2d, test_set_2d, data_mean_2d, data_std_2d, dim_to_ignore_2d, dim_to_use_2d = data_utils_1.create_2d_data( actions, FLAGS.data_dir, rcams )
  ...
  with tf.Session(config=tf.ConfigProto( device_count = device_count )) as sess:
    ...
    for key2d in test_set_2d.keys():
      ...
      for bidx in range( len(enc_in) ):
        ...
        # denormalize
        enc_in[bidx]  = data_utils_1.unNormalizeData(  enc_in[bidx], data_mean_2d, data_std_2d, dim_to_ignore_2d )
        dec_out[bidx] = data_utils_1.unNormalizeData( dec_out[bidx], data_mean_3d, data_std_3d, dim_to_ignore_3d )
        poses3d = data_utils_1.unNormalizeData( poses3d, data_mean_3d, data_std_3d, dim_to_ignore_3d )
  ...
  for i in np.arange( nsamples ):
    ...
    p2d = enc_in[exidx,:]
    viz_1.show2Dpose( p2d, ax1 )
    ax1.invert_yaxis()

    # Plot 3d gt
    ax2 = plt.subplot(gs1[subplot_idx], projection='3d')
    p3d = dec_out[exidx,:]
    viz_1.show3Dpose( p3d, ax2 )

    # Plot 3d predictions
    ax3 = plt.subplot(gs1[subplot_idx+1], projection='3d')
    p3d = poses3d[exidx,:]
    viz_1.show3Dpose( p3d, ax3, lcolor="#9b59b6", rcolor="#2ecc71" )
